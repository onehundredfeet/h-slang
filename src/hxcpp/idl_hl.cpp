...
HL_PRIM pref<gfx::IModule>* HL_NAME(Device_loadModule1)(pref<gfx::IDevice>* _this, vstring * moduleName) {
	const char* moduleName__cstr = (moduleName == nullptr) ? "" : hl_to_utf8( moduleName->bytes ); // Should be garbage collected
	auto ___retvalue = alloc_ref((_unref(_this)->loadModule(moduleName__cstr)),Module);
	return (___retvalue);
}
DEFINE_PRIM(_IDL, Device_loadModule1, _IDL _STRING);
